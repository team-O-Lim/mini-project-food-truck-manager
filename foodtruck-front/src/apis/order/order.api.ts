import type { OrderDetailResponse, OrderListResponse, OrderUpdateRequest } from "@/types/order/order.dto";
...
export const orderApi = {
  ...
  // 주문 수정
  updateOrder: async (orderId: number, request: OrderUpdateRequest): Promise<OrderDetailResponse> => {
    const res = await privateApi.put<ApiResponse<OrderDetailResponse>> (
      ORDER_PATH.UPDATE(orderId), request
    );
      return res.data.data;
  },
